...
import random
import os
import json  # 新增：JSON解析核心模块
from bs4 import BeautifulSoup


# ========== 替换正则的核心函数：JSON解析删除note_card ==========
def process_json_remove_note_card(raw_data_str):
    """
    解析JSON字符串，删除每个item里的note_card字段（替换原正则逻辑）
    :param raw_data_str: 原始JSON字符串数据
    :return: 处理后的JSON字符串（已删除note_card），处理成功返回字符串，失败返回空字符串
    """
    try:
        ...
        # 3. 遍历删除每个item的note_card
        processed_count = 0
        for item in items:
            if item.pop("note_card", None):  # 安全删除，不存在则返回None
                processed_count += 1

        # 4. 转回格式化JSON字符串（保留中文、排版）
        processed_json_str = json.dumps(
            json_data,
            ensure_ascii=False,
            indent=4
        )

        print(f"✅ JSON处理完成！共删除 {processed_count} 个item的note_card字段")
        return processed_json_str

    except json.JSONDecodeError:
        print("❌ 错误：原始数据不是合法的JSON格式，无法用JSON解析处理")
        return ""
    except Exception as e:
        print(f"❌ JSON处理出错：{str(e)}")
        return ""


def extract_outer_id_and_token(raw_data_str):
    """
    先通过JSON解析删除note_card，再提取最外层的id和xsec_token
    :param raw_data_str: 原始JSON字符串数据
    :return: 提取到的(id, xsec_token)列表
    """
    # 第一步：用JSON解析删除note_card（替换原正则逻辑）
    cleaned_json_str = process_json_remove_note_card(raw_data_str)
    if not cleaned_json_str:  # JSON处理失败，直接返回空列表
        return []

    # 第二步：从处理后的JSON字符串中提取id和xsec_token（正则仅用于提取，无干扰）
    target_pattern = r'\{\s*"id":\s*"([^"]+)"[,}].*?"xsec_token":\s*"([^"]+)"'
    matches = re.findall(target_pattern, cleaned_json_str, flags=re.DOTALL)
...
if __name__ == "__main__":
    # 1. 读取JSON数据文件（替换为你的实际路径）
    file_path = r"xiaohongshu_data_2.txt"  # 原始JSON数据文件
    raw_data = ""
    try:
    ...
    # 2. 提取id和xsec_token（已改用JSON解析删除note_card）
    result = extract_outer_id_and_token(raw_data)
    xhs_urls = []
    if result:
        print("\n✅ 提取到最外层的id和xsec_token：")
